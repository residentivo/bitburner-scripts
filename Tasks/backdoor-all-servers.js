...
export let main = async ns => {
    ...
    try {
        ...
        let toBackdoor = await getNsDataThroughFile(ns, `${JSON.stringify(hackableServers)}.filter(s => !(ns.getServer(s)?.backdoorInstalled ?? false))`, '/Temp/servers-to-backdoor.txt');
