/**
 * solver-mini.js - Minimal contract solver (no file I/O, minimal RAM)
 * Receives batch file path as arg, solves all contracts in the batch.
 * Designed to be spawned via getNsDataThroughFile for minimal RAM overhead.
 *
 * This is a slim alternative to contractor.js.solver.js that:
 * - Reads batch from temp file (small arg = small RAM)
 * - Contains only the solver logic (no parsing boilerplate)
 * - Writes results to a temp log file since ns.tprint may not be visible from temp scripts
 */

export async function main(ns) {
    const batchFile = ns.args[0];
    const raw = ns.read(batchFile);
    if (!raw || raw === '') { ns.tprint('ERROR: Cannot read ' + batchFile); return; }

    const contracts = JSON.parse(raw, function(k, v) {
        return typeof v === 'string' && v.startsWith('__BIGINT__') ? BigInt(v.slice(10)) : v;
    });

    let solved = 0, failed = 0, skipped = 0;

    for (const c of contracts) {
        if (!c || !c.type || c.data == null) { skipped++; continue; }
        try {
            ...
            if (ok) {
                solved++;
                ns.tprint('Solved ' + c.contract + ' on ' + c.hostname + ': ' + JSON.stringify(answer).substring(0, 80));
            } else {
                failed++;
                ns.tprint('WRONG: ' + c.contract + ' on ' + c.hostname + ': ' + JSON.stringify(answer).substring(0, 80));
            }
        } catch (e) {
            failed++;
            ns.tprint('ERR: ' + c.contract + ': ' + e.toString().substring(0, 80));
        }
        await ns.sleep(10);
    }
...
const SOLVERS = [
    { name: 'Find Largest Prime Factor', fn: function(d) {
        if (typeof d === 'bigint') { var n=d,f=2n; while(n>(f-1n)*(f-1n)){while(n%f===0n)n=n/f;++f;} return n===1n?f-1n:n; }
        var n=d,f=2; while(n>(f-1)*(f-1)){while(n%f===0)n=Math.round(n/f);++f;} return n===1?f-1:n;
    }},
    ...
    { name: 'Merge Overlapping Intervals', fn: function(d) { var iv=d.slice();iv.sort(function(a,b){return a[0]-b[0];});var res=[];var s=iv[0][0],e=iv[0][1];for(var i=0;i<iv.length;i++){if(iv[i][0]<=e)e=Math.max(e,iv[i][1]);else{res.push([s,e]);s=iv[i][0];e=iv[i][1];}}res.push([s,e]);var o=[];res.forEach(function(x){o.push('['+x.toString()+']');});return o.join(',').replace(/\s/g,''); }},
    ...
    { name: 'Algorithmic Stock Trader I', fn: function(d) { var p=d.map(function(v){return Number(v);});var mc=0,ms=0;for(var i=1;i<p.length;i++){mc=Math.max(0,mc+=p[i]-p[i-1]);ms=Math.max(mc,ms);}return ms; }},
    { name: 'Algorithmic Stock Trader II', fn: function(d) { var p=0;for(var i=1;i<d.length;i++)p+=Math.max(d[i]-d[i-1],0);return p; }},
    { name: 'Algorithmic Stock Trader III', fn: function(d) { var h1=Number.MIN_SAFE_INTEGER,h2=Number.MIN_SAFE_INTEGER,r1=0,r2=0;for(var i=0;i<d.length;i++){var price=d[i];r2=Math.max(r2,h2+price);h2=Math.max(h2,r1-price);r1=Math.max(r1,h1+price);h1=Math.max(h1,-price);}return r2; }},
    { name: 'Algorithmic Stock Trader IV', fn: function(d) { var k=d[0],pr=d[1],len=pr.length;if(len<2)return 0;if(k>len/2){var res=0;for(var i=1;i<len;i++)res+=Math.max(pr[i]-pr[i-1],0);return res;}var hold=[],rele=[];hold.length=k+1;rele.length=k+1;for(var i=0;i<=k;i++){hold[i]=Number.MIN_SAFE_INTEGER;rele[i]=0;}for(var i=0;i<len;i++){var cur=pr[i];for(var j=k;j>0;j--){rele[j]=Math.max(rele[j],hold[j]+cur);hold[j]=Math.max(hold[j],rele[j-1]-cur);}}return rele[k]; }},
    ...
    { name: 'Sanitize Parentheses in Expression', fn: function(d) { var l=0,r=0,res=[];for(var i=0;i<d.length;i++){if(d[i]==='(')++l;else if(d[i]===')')l>0?--l:++r;}function dfs(p,idx,l,r,s,sol,res){if(s.length===idx){if(l===0&&r===0&&p===0){for(var i=0;i<res.length;i++)if(res[i]===sol)return;res.push(sol);}return;}if(s[idx]==='('){if(l>0)dfs(p,idx+1,l-1,r,s,sol,res);dfs(p+1,idx+1,l,r,s,sol+s[idx],res);}else if(s[idx]===')'){if(r>0)dfs(p,idx+1,l,r-1,s,sol,res);if(p>0)dfs(p-1,idx+1,l,r,s,sol+s[idx],res);}else{dfs(p,idx+1,l,r,s,sol+s[idx],res);}}dfs(0,0,l,r,d,'',res);return res; }},
    { name: 'Find All Valid Math Expressions', fn: function(d) { var num=d[0],target=d[1];function helper(res,path,num,tar,pos,eval,mult){if(pos===num.length){if(tar===eval)res.push(path);return;}for(var i=pos;i<num.length;i++){if(i!=pos&&num[pos]==='0')break;var cur=parseInt(num.substring(pos,i+1));if(pos===0)helper(res,path+cur,num,tar,i+1,cur,cur);else{helper(res,path+'+'+cur,num,tar,i+1,eval+cur,cur);helper(res,path+'-'+cur,num,tar,i+1,eval-cur,-cur);helper(res,path+'*'+cur,num,tar,i+1,eval-mult+mult*cur,mult*cur);}}}if(!num||num.length===0)return[];var result=[];helper(result,'',num,target,0,0,0);return result; }},
    ...
    { name: 'Largest Rectangle in a Matrix', fn: function(d) { var hist=Array.from({length:d.length},function(){return Array(d[0].length).fill(0);});for(var i=0;i<d[0].length;i++){var ct=0;for(var j=0;j<d.length;j++){if(d[j][i]==0)ct++;else ct=0;hist[j][i]=ct;}}var maxA=0,maxL=0,maxR=0,maxU=0,maxD=0;for(var i=0;i<hist.length;i++){var row=hist[i];for(var j=0;j<row.length;j++){if(row[j]==0)continue;var l=j,r=j;while(l-1>=0&&row[l-1]>=row[j])l--;while(r+1<row.length&&row[r+1]>=row[j])r++;if((r-l+1)*row[j]>maxA){maxA=(r-l+1)*row[j];maxL=l;maxR=r;maxU=i-row[j]+1;maxD=i;}}}return[[maxU,maxL],[maxD,maxR]]; }},
    { name: 'Square Root', fn: function(d) { var n;if(typeof d==='bigint')n=d;else if(typeof d==='string'){var s=d.startsWith('__BIGINT__')?d.slice(10):d;try{n=BigInt(s);}catch(e){return null;}}else if(typeof d==='number'&&!isNaN(d)&&isFinite(d))return Math.floor(Math.sqrt(d));else return null;if(n<0n)return null;if(n<2n)return n;var lo=1n,hi=n;while(lo<=hi){var mid=(lo+hi)/2n,ms=mid*mid;if(ms===n)return mid;if(ms<n)lo=mid+1n;else hi=mid-1n;}return hi; }},
    ...
    { name: 'HammingCodes: Encoded Binary to Integer', fn: function(d) { var enc=d.split('').map(function(v){return parseInt(v);});var m=0,n=enc.length;while(Math.pow(2,m)<m+n+1)m++;var pn=0;for(var i=0;i<n;i++){var expected=0;for(var j=0;j<m;j++)if(i&(1<<j))expected^=enc[Math.pow(2,j)];if(enc[i]!==expected)pn^=i;}if(pn!==0&&pn<n)enc[pn]=1-enc[pn];var dataBits=[];for(var i=1;i<n;i++)if((i&(i-1))!==0)dataBits.push(enc[i]);dataBits.reverse();return parseInt(dataBits.join(''),2); }},
    ...
    { name: 'Total Primes in Range', fn: function(d) { var low=Number(d[0]),high=Number(d[1]);if(high<2)return 0;if(low<2)low=2;if(high<=10000000){var s=new Array(high+1).fill(true);s[0]=s[1]=false;for(var i=2;i*i<=high;i++)if(s[i])for(var j=i*i;j<=high;j+=i)s[j]=false;var c=0;for(var i=low;i<=high;i++)if(s[i])c++;return c;}var sqH=Math.ceil(Math.sqrt(high)),sp=[],ss=new Array(sqH+1).fill(true);ss[0]=ss[1]=false;for(var i=2;i<=sqH;i++){if(ss[i]){sp.push(i);for(var j=i*i;j<=sqH;j+=i)ss[j]=false;}}var rs=high-low+1,seg=new Array(rs).fill(true);for(var p=0;p<sp.length;p++){var pr=sp[p],st=Math.max(pr,Math.ceil(low/pr))*pr;for(var j=st;j<=high;j+=seg)j-=0;var st2=Math.max(pr*Math.ceil(low/pr),pr*2);for(var j=st2;j<=high;j+=pr)seg[j-low]=false;}var c=0;for(var i=0;i<rs;i++)if(seg[i])c++;return c; }},
    { name: 'Total Number of Primes', fn: function(d) { return SOLVERS.find(function(s){return s.name==='Total Primes in Range';}).fn(d); }},
];

function findAnswer(contract) {
    const solver = SOLVERS.find(function(s) { return s.name === contract.type; });
    if (!solver) return null;
    return solver.fn(contract.data);
}
