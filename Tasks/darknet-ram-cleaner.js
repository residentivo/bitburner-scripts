...
export async function main(ns) {
    ...
    while (true) {
        try {
            ...
            const password = await solvePassword(serverDetails.passwordHint || "", serverDetails.data || "", darknetHost);
...
// Copy of solvePassword function from darknet.js for password solving
function solvePassword(hint, hintData, hostname = '') {
    ...
    if (h.includes('default') || h.includes('factory') || h.includes('never changed') ||
        ...
        return [...new Set([
            ...
            'open', 'login', 'unlock', 'access', 'secret', 'test', 'user', 'demo', 'backup',
    ...
    if (h.includes('master') || h.includes('true master') || h.includes('may pass'))
        return [...new Set([
            ...
            'wisom', 'enlightenment', 'discipline', 'skill', 'expert',
...
// Helper function for permutations (used by solvePassword)
...
// Helper array for common passwords (used by solvePassword)
...
// Helper array for mountain passwords (used by solvePassword)
...
// Helper array for extended passwords (used by solvePassword)
