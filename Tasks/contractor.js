 ...
 * Self-contained: scans servers, reads contracts, solves and submits.
...
function solve(type, data) {
    ...
    if (type === 'Total Ways to Sum') {
        var w=[1]; w.length=data+1; w.fill(0,1); for(var i=1;i<data;i++)for(var j=i;j<=data;j++)w[j]+=w[j-i]; return w[d];
    }
    ...
    if (type === 'Array Jumping Game') {
        var n=data.length,i=0,r=0;for(;i<n&&i<=r;i++)r=Math.max(i+d[i],r);return i===n?1:0;
    }
    ...
    if (type === 'Algorithmic Stock Trader II') {
        var p=0;for(var i=1;i<d.length;i++)p+=Math.max(d[i]-d[i-1],0);return p.toString();
    }
    ...
    if (type === 'Minimum Path Sum in a Triangle') {
        var n=data.length,dp=d[n-1].slice();for(var i=n-2;i>-1;i--)for(var j=0;j<d[i].length;j++)dp[j]=Math.min(dp[j],dp[j+1])+d[i][j];return dp[0];
    }
    if (type === 'Unique Paths in a Grid I') {
        var n=d[0],m=d[1],cr=[];cr.length=n;for(var i=0;i<n;i++)cr[i]=1;for(var row=1;row<m;row++)for(var i=1;i<n;i++)cr[i]+=cr[i-1];return cr[n-1];
    }
    if (type === 'Unique Paths in a Grid II') {
        var og=[];og.length=d.length;for(var i=0;i<og.length;i++)og[i]=d[i].slice();for(var i=0;i<og.length;i++)for(var j=0;j<og[0].length;j++){if(og[i][j]==1)og[i][j]=0;else if(i==0&&j==0)og[0][0]=1;else og[i][j]=(i>0?og[i-1][j]:0)+(j>0?og[i][j-1]:0);}return og[og.length-1][og[0].length-1];
    }
    ...
    if (type === 'Find All Valid Math Expressions') {
        var num=d[0],target=d[1];if(!num||num.length===0)return[];var result=[];findExpr(result,'',num,target,0,0,0);return result;
        function findExpr(res,path,s,tar,pos,curTotal,mult){if(pos===s.length){if(tar===curTotal)res.push(path);return;}for(var i=pos;i<s.length;i++){if(i!=pos&&s[pos]==='0')break;var cur=parseInt(s.substring(pos,i+1));if(pos===0)findExpr(res,path+cur,s,tar,i+1,curTotal,cur);else{findExpr(res,path+'+'+cur,s,tar,i+1,curTotal+cur,cur);findExpr(res,path+'-'+cur,s,tar,i+1,curTotal-cur,-cur);findExpr(res,path+'*'+cur,s,tar,i+1,curTotal-mult+mult*cur,mult*cur);}}}
    }
    ...
    if (type === 'Square Root') {
        var n;if(typeof data==='bigint')n=data;else if(typeof data==='string'){var s=data.startsWith('__BIGINT__')?data.slice(10):data;try{n=BigInt(s);}catch(e){return null;}}else if(typeof data==='number'&&!isNaN(data)&&isFinite(data))return Math.round(Math.sqrt(data)).toString();else return null;if(n<0n)return null;if(n<2n)return n.toString();var lo=1n,hi=n;while(lo<=hi){var mid=(lo+hi)/2n,ms=mid*mid;if(ms===n)return mid.toString();if(ms<n)lo=mid+1n;else hi=mid-1n;}var floorSq=hi*hi;var ceilSq=(hi+1n)*(hi+1n);var distToFloor=n-floorSq;var distToCeil=ceilSq-n;return distToFloor<=distToCeil?hi.toString():(hi+1n).toString();
    }
...
export async function main(ns) {
    ...
    while (queue.length > 0) {
        ...
        for (const s of ns.scan(current)) {
            if (!allServers.has(s)) { allServers.add(s); queue.push(s); }
        }
    }

    // Find contracts
    const contracts = [];
    for (const srv of [...allServers].sort()) {
        try {
            for (const ct of ns.ls(srv, '.cct')) {
                contracts.push({ contract: ct, hostname: srv });
            }
        } catch (_) {}
    }

    if (contracts.length === 0) return ns.tprint("No contracts found.");
    ns.tprint("Found " + contracts.length + " contracts across " + allServers.size + " servers.");

    // Get type and data for each contract
    let ready = 0;
    ...
    ns.tprint(ready + "/" + contracts.length + " contracts readable.");

    const todo = contracts.filter(c => c.type && c.data != null);
    ...
    for (const c of todo) {
        ...
        try {
            ...
            if (ok && ok !== "") {
                solved++;
                ns.tprint("SOLVED: " + c.contract + " on " + c.hostname + " (" + c.type + ") -> " + ok.trim().split("\n")[0]);
            } else {
                failed++;
                // Don't spam wrong answers in log - just count them
            }
        } catch (e) {
            failed++;
        }
