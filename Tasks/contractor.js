...
export async function main(ns) {
    ...
    // Build payload and split into batches
    const BATCH_SIZE = 10;
    const batches = [];
    for (let i = 0; i < allContracts.length; i += BATCH_SIZE) {
        batches.push(allContracts.slice(i, i + BATCH_SIZE));
    }
    ns.tprint(`Solving ${dataCount} contracts in ${batches.length} batches (in-process, no spawn)...`);

    let totalSolved = 0, totalFailed = 0, totalSkipped = 0;
    for (let bi = 0; bi < batches.length; bi++) {
        ns.tprint(`Batch ${bi + 1}/${batches.length}: ${batches[bi].length} contracts`);
...
// --- Inline solver functions (copied from contractor.js.solver.js) ---
...
const SOLVERS = [
    ...
    { name: 'Merge Overlapping Intervals', fn: function(d) { var iv=d.slice();iv.sort(function(a,b){return a[0]-b[0];});var res=[];var s2=iv[0][0],e=iv[0][1];for(var i=0;i<iv.length;i++){if(iv[i][0]<=e)e=Math.max(e,iv[i][1]);else{res.push([s2,e]);s2=iv[i][0];e=iv[i][1];}}res.push([s2,e]);var o=[];res.forEach(function(x){var ss=x.toString();o.push('['+ss+']');});return o.join(',').replace(/\s/g,''); }},
    ...
    { name: 'Compression II: LZ Decompression', fn: function(d) { var p='',i=0;while(i<d.length){var ll=d.charCodeAt(i)-0x30;if(ll<0||ll>9||i+1+ll>d.length)break;p+=d.substring(i+1,i+1+ll);i+=1+ll;if(i>=d.length)break;var bl=d.charCodeAt(i)-0x30;if(bl<0||bl>9)break;else if(bl===0){i++;continue;}if(i+1>=d.length)break;var bo=d.charCodeAt(i+1)-0x30;if(bo<1||bo>9)break;if(bo>p.length)break;for(var j=0;j<bl;j++)p+=p[p.length-bo];i+=2;}return p; }},
