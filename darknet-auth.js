/**
 * darknet-auth.js — Dedicated darknet authentication solver
 * Usage: run darknet-auth.js <target_server>
 * 
 * Receives a server name as parameter, attempts to authenticate
 * using all known password solving strategies, then scp+exec
 * propagation scripts if auth succeeds.
 * 
 * This is spawned by darknet.js per-peer to avoid tick timeouts
 * from running auth for multiple servers in a single loop.
 */
...
export async function main(ns) {
    ...
    if (!target) {
        ns.print(`[dnet-auth] ERROR: no target server specified`)
        ns.print(`[dnet-auth] Usage: run darknet-auth.js <server>`)
        return
    }
    ...
    let details
    try {
        details = await ns.dnet.getServerDetails(target)
    } catch (e) {
        ns.print(`[dnet-auth] ${target} getServerDetails ERROR: ${e}`)
        return
    }

    ns.print(`[dnet-auth] ${target} details: online=${details.isOnline} connected=${details.isConnectedToCurrentServer} hasSession=${details.hasSession} hint="${details.passwordHint}"`)

    if (!details.isOnline || !details.isConnectedToCurrentServer) {
        ns.print(`[dnet-auth] ${target} SKIPPED (offline or not connected)`)
        return
    }

    let hasSession = details.hasSession

    // Step 2: If no session, authenticate
    if (!hasSession) {
        const hint = details.passwordHint || ''
        const data = details.data || ''
        const candidates = solvePassword(hint, data, target)

        ns.print(`[dnet-auth] ${target} auth: ${candidates.length} candidates, hint="${hint}"`)
        ...
        let bleedPasswords = []
        try {
            const logs = await ns.dnet.heartbleed(target)
            if (logs && logs.length > 0) {
                ns.print(`[dnet-auth] ${target} BLEED: ${JSON.stringify(logs).substring(0, 300)}`)
        ...
        if (bleedPasswords.length > 0) {
            ...
            ns.print(`[dnet-auth] ${target} heartbleed: ${bleedCapped.length} candidates`)
            for (const pw of bleedCapped) {
                try {
                    const r = await ns.dnet.authenticate(target, pw)
                    if (r.success) { authed = true; hasSession = true; ns.print(`[dnet-auth] ${target} AUTH SUCCESS (bleed) with "${pw}"`); break }
                } catch (e) { /* try next */ }
            }
        }
        ...
        if (!authed && candidates.length > 0) {
            ...
            if (candidates.length > 50000) {
                ns.print(`[dnet-auth] ${target} auth: CAPPING ${candidates.length} → ${maxAuthAttempts}`)
            }
            for (let i = 0; i < maxAuthAttempts; i++) {
                ...
                try {
                    ...
                    if (r.success) {
                        authed = true
                        hasSession = true
                        ns.print(`[dnet-auth] ${target} AUTH SUCCESS with "${pw}" (attempt ${i+1}/${maxAuthAttempts})`)
                        break
                    }
                } catch (e) { /* try next */ }
            }
            if (!authed && candidates.length > maxAuthAttempts) {
                ns.print(`[dnet-auth] ${target} auth: gave up after ${maxAuthAttempts}/${candidates.length}`)
            }
        }

        if (!authed) {
            const bleedInfo = bleedPasswords.length > 0 ? ` | bleed: ${bleedPasswords.length}` : ' | no bleed'
            ns.print(`[dnet-auth] ${target} AUTH FAILED: tried ${candidates.length}${bleedInfo}`)
        }
    } else {
        ns.print(`[dnet-auth] ${target} already has session`)
    }

    // Step 3: ALWAYS scp scripts to target (even if auth failed)
    try {
        const scp1 = await ns.scp(SCRIPT_NAME, target, host)
        const scp2 = await ns.scp('darknet-ram.js', target, host)
        const scp3 = await ns.scp(EXTRACTOR, target, host)
        ns.print(`[dnet-auth] ${target} SCP: darknet.js=${scp1} ram.js=${scp2} extractor=${scp3}`)
    } catch (e) {
        ns.print(`[dnet-auth] ${target} SCP ERROR: ${e}`)
    }

    // Step 4: exec darknet.js on target
    try {
        const targetProcs = ns.ps(target)
        const alreadyRunning = targetProcs.some(p => p.filename === SCRIPT_NAME)
        if (alreadyRunning) {
            ns.print(`[dnet-auth] ${target} already running darknet.js`)
        } else {
            ...
            const pid = ns.exec(SCRIPT_NAME, target, threads)
            ns.print(`[dnet-auth] ${target} EXEC: pid=${pid} scriptRam=${scriptRam} freeRam=${freeRam}`)
            if (pid === 0) {
                ns.print(`[dnet-auth] ${target} EXEC FAILED: ns.exec returned 0`)
            }
        }
        // Also run extractor
        const extractorRunning = targetProcs.some(p => p.filename === EXTRACTOR)
        if (!extractorRunning) {
            const extPid = ns.exec(EXTRACTOR, target, 1)
            ns.print(`[dnet-auth] ${target} EXTRACTOR exec: pid=${extPid}`)
        }
    } catch (e) {
        ns.print(`[dnet-auth] ${target} EXEC ERROR: ${e}`)
    }

    ns.print(`[dnet-auth] ${target} DONE (hasSession=${hasSession})`)
}
