...
export async function workForMegacorpFactionInvite(ns, factionName, waitForInvite) {
    ...
    while (((currentReputation = (await getCompanyReputation(ns, companyName))) < repRequiredForFaction) && !player.factions.includes(factionName)) {
        ...
        const toJobField = (r) => r === "it" ? "IT" : r === "software" ? "Software" : r;
        if (currentJobTier < bestJobTier || currentRole != bestRoleName) {
            if (await getNsDataThroughFile(ns, `ns.singularity.applyToCompany('${companyName}','${toJobField(bestRoleName)}')`, '/Temp/apply-to-company.txt'))
        ...
        if (await getNsDataThroughFile(ns, `ns.singularity.applyToCompany('${companyName}','${toJobField(currentRole)}')`, '/Temp/apply-to-company.txt'))
        ...
        if (lastStatus != status || (Date.now() - lastStatusUpdateTime) > statusUpdateInterval) {
            if (!backdoored) // Check if an external script has backdoored this company's server yet. If so, it affects our ETA. (Don't need to check again once we discover it is)
                backdoored = await getNsDataThroughFile(ns, `ns.getServer('${serverByCompany[companyName]}')?.backdoorInstalled ?? false`, '/Temp/company-is-backdoored.txt');
