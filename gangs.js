...
async function mainLoop(ns) {
    ...
    if (!territoryTickDetected) { // Detect the first territory tick by watching for other gang's territory power to update.
        const otherGangInfo = await getNsDataThroughFile(ns, 'ns.gang.getAllGangInformation()', '/Temp/gang-other-gang-info.txt'); // Returns dict of { [gangName]: { "power": Number, "territory": Number } }
...
async function optimizeGangCrime(ns, myGangInfo) {
    ...
    if (ownedSourceFiles[4] > 0) {
        try { factionRep = await getNsDataThroughFile(ns, `ns.singularity.getFactionRep('${myGangFaction}')`, `/Temp/gang-faction-rep.txt`); }
...
async function enableOrDisableWarfare(ns, myGangInfo) {
    ...
    const otherGangs = await getNsDataThroughFile(ns, 'ns.gang.getAllGangInformation()', '/Temp/gang-other-gang-info.txt'); // Returns dict of { [gangName]: { "power": Number, "territory": Number } }
