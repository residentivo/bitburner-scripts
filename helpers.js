...
export async function runCommand_Custom(ns, fnRun, command, fileName, args = [], verbose = false, maxRetries = 5, retryDelayMs = 50) {
    ...
    let script = `export async function main(ns) { try { ` +
        (verbose ? `let output = ${command}; let s = typeof output === 'string' ? output : JSON.stringify(output); if (s.length > 300) { ns.print('[output too large, ' + s.length + ' chars - suppressed]'); } else { ns.tprint(s); }` : command) +
...
export async function getActiveSourceFiles_Custom(ns, fnGetNsDataThroughFile, includeLevelsFromCurrentBitnode = true) {
    ...
    let dictSourceFiles;
    try { dictSourceFiles = await fnGetNsDataThroughFile(ns, `Object.fromEntries(ns.singularity.getOwnedSourceFiles().map(sf => [sf.n, sf.lvl]))`, tempFile); } catch (e) { log(ns, `WARN: getActiveSourceFiles failed: ${String(e)}`); }
    ...
    if (includeLevelsFromCurrentBitnode) {
        ...
        let bitNodeN;
        try {
            bitNodeN = (typeof ns.getResetInfo === 'function') ? ns.getResetInfo().currentNode :
                (await fnGetNsDataThroughFile(ns, 'ns.getPlayer()', '/Temp/player-info.txt')).bitNodeN;
        } catch { bitNodeN = 1; }
